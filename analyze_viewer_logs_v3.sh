...
# 输出 CSV 表头
echo "host,channel,index,start_time,first_audio_time,audio_latency_ms,first_video_time,video_latency_ms,pull_success,stable_audio,stable_video" > "$output_csv"
...
for log_file in "$log_dir"/viewer_*.log; do
    ...
    # 转换为时间戳
    start_ms="${results[3]}"
    first_audio_ms="${results[4]}"
    first_video_ms="${results[5]}"
    ...
    # 写入结果
    echo "$PUBLIC_IP,$channel,$index,'$start_time','$first_audio_time',$audio_latency,'$first_video_time',$video_latency,$pull_success,$stable_audio,$stable_video" >> "$output_csv"
done
